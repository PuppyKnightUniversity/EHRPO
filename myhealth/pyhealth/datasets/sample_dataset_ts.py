...
class SampleBaseDataset_ts(Dataset):
    ...
    def get_all_tokens_ts(
        self, remove_duplicates: bool = True, sort: bool = True
    ) -> List[str]:
        """Gets all tokens in the samples.
        Note:
            In order to handle time series, we need to build a dict for all tokens.
            concat all type of medical codes together instead of handling them separately
        ...
        """
        tokens = []
        # TODO: build all tokens
        for sample in self.samples:
            visit_code_list = sample["visit_sequence_codes"]
            for visit_code in visit_code_list:
                tokens += visit_code
        if remove_duplicates:
            tokens = list(set(tokens))
        if sort:
...
class SampleSignalDataset_ts(SampleBaseDataset_ts):
...
class SampleEHRDataset_ts(SampleBaseDataset_ts):
    ...
    def _validate(self) -> Dict:
        ...
        Modify:
            need support for time series data
        """
        ...
        4. For each key, check if it is either:
            ...
            - a list of list of visit objects
        Note that a value is either float, int, str, or visit; a vector is a list of float 
        ...
        for key in keys:
            ...
            - a list of visit objects (level=1)
            - a list of list of visit objects (level=2)
            """
            ...
            assert (
                    types == set([str]) or len(types.difference(set([int, float, Visit]))) == 0
            ...
            if level == 0:
                ...
                # a single vector, a list of codes, or a list of visit objects
                if type_ in [float, int]:
                    ...
                    # a list of visit objects
                    input_info[key] = {"type": type_, "dim": 1,"len": 'unknow'}
                else:
                    # a list of codes
                    # note that dim is different from level here
                    input_info[key] = {"type": type_, "dim": 2}
            elif level == 2:
                # a list of vectors, a list of list of codes, or a list of list of visit objects
                if type_ in [float, int]:
                    ...
                    # a list of list of visit objects
                    input_info[key] = {"type": type_, "dim": 2, "len": 'unknow'}
                else:
                    # a list of list of codes
                    # note that dim is different from level here
...
if __name__ == "__main__":
    ...
    dataset = SampleEHRDataset_ts(samples=samples)
