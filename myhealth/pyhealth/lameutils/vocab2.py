...
class VocabEntry(object):
    ...
    def encode_notes(self, sents: List[List[str]]) -> torch.Tensor:
        """ Convert list of sentences (words) into tensor with necessary padding for
        shorter sentences.
        ...
        @returns sents_var: tensor of (max_sentence_length, batch_size)
        """
        note_lens = [len(s) for s in sents]
        word_ids = self.words2indices(sents)
        sents_t = pad_sents(word_ids, self['<pad>'])
        return sents_t, note_lens

    # def multi_list_to_input_tensor(self, sents: List[List[str]], device: torch.device) -> torch.Tensor:
    #     """ Convert list of sentences (words) into tensor with necessary padding for 
    #     shorter sentences.
