...
class Patient:
    ...
    def sort_visits(self):
        ''' Sorts the visits list in ascending order.
        '''
        visits = OrderedDict(sorted(self.visits.items(), key=lambda x: attrgetter('encounter_time')(x[1])))
        self.visits = visits

    def check_visits(self):
        ''' Checks the visits list for validity.
        '''
        for i in range(len(self.visits)-1):
            visit = self.get_visit_by_index(i)
            next_visit = self.get_visit_by_index(i+1)
            assert visit.encounter_time < next_visit.encounter_time
    ...
    def add_visit(self, visit: Visit) -> None:
        ...
        Alert:
            must make sure the sequence of visits!
        """
        ...
        self.sort_visits()
        # rebuild index
        temp_index_to_visit_id = dict()
        for i, (k, v) in enumerate(self.visits.items()):
            temp_index_to_visit_id[i] = k
        self.index_to_visit_id = temp_index_to_visit_id
        self.check_visits()
