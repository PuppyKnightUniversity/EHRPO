# This is an modified version for time series handle model
...
from pyhealth.datasets import SampleBaseDataset_ts
...
class BaseModel_ts(ABC, nn.Module):
    ...
    def __init__(
        self,
        dataset: SampleBaseDataset_ts,
        label_key: str,
        mode: Optional[str] = None,
        pretrained_emb: str = None
    ):
        super(BaseModel_ts, self).__init__()
        if mode is not None:
    ...
    def get_feature_tokenizers(self, special_tokens=None) -> Dict[str, Tokenizer]:
        ...
        feature_tokenizers = {}
        feature_tokenizers['all'] = Tokenizer(tokens=self.dataset.get_all_tokens_ts(),
    ...
    # TODO: for timeseries, build A unified feature_transform_layer rather than 3 types
    def add_feature_transform_layer_ts(self, special_tokens=None):

        # build tokenizer for all medical codes
        if special_tokens is None:
            special_tokens = ["<pad>", "<unk>"]
        tokenizer = Tokenizer(
            tokens=self.dataset.get_all_tokens_ts(),
            special_tokens=special_tokens,
        )
        self.feat_tokenizers['all'] = tokenizer

        # build embedding layer
        self.embeddings['all'] = nn.Embedding(
            tokenizer.get_vocabulary_size(),
            self.embedding_dim,
            padding_idx=tokenizer.get_padding_index(),
        )
