# This is a modified RNN version for time series
from typing import Dict, List, Optional, Tuple
import torch
import torch.nn as nn
import torch.nn.utils.rnn as rnn_utils

from pyhealth.datasets import SampleEHRDataset_ts
from pyhealth.models import BaseModel_ts

# VALID_OPERATION_LEVEL = ["visit", "event"]


class RNNLayer_ts(nn.Module):
    ...
    def __init__(
        ...
        super(RNNLayer_ts, self).__init__()
...
# Todo: modify for time series
class RNN_ts(BaseModel_ts):
    ...
    def __init__(
        ...
        **kwargs
        ...
        # actually, we use only ONE tokenizer for all medical codes
        self.add_feature_transform_layer_ts()
        self.rnn = RNNLayer_ts(input_size=embedding_dim, hidden_size=hidden_dim, **kwargs)
    ...
    def forward(self, **kwargs) -> Dict[str, torch.Tensor]:
        ...
        """
        patient_features = kwargs['visit_sequence_codes']
        x = self.feat_tokenizers['all'].batch_encode_3d(patient_features)
        # (patient, visit, event)
        x = torch.tensor(x, dtype=torch.long, device=self.device)
        # (patient, visit, event, embedding_dim)
        x = self.embeddings['all'](x)
        # (patient, visit, embedding_dim)
        x = torch.sum(x, dim=2)
        # (patient, visit)
        mask = torch.any(x != 0, dim=2) # define effective time step in a patient's visit
        _, x = self.rnn(x, mask)
        patient_emb = x
        logits = self.fc(patient_emb)
        # obtain y_true, loss, y_prob
        print(kwargs[self.label_key])
        label_list = [ str(label[0]) for label in kwargs[self.label_key]]
        print(label_list)
        y_true = self.prepare_labels(label_list, self.label_tokenizer)
